                ...
                if (dbconnection != null)
                {
                    if (bankRecListModel.BankreconcilationList.Count > 0)
                    {
                        for (int i = 0; i < bankRecListModel.BankreconcilationList.Count; i++)
                        {
                            ...
                            var statusData = await SqlHelper.SqlHelper.ExecuteDatasetAsync(transaction, "usp_BankReconcilationSave", param);
                            if (statusData != null && statusData.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (!Convert.ToBoolean(statusData.Tables[0].Rows[0]["Status"]))
                                {
                                    ...
                                    i = bankRecListModel.BankreconcilationList.Count;
                                }
                            }
                            else
                            {
