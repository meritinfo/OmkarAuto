...
@Component({
  selector: 'app-tyreregrouprcvdrpt',
  templateUrl: './tyreregrouprcvdrpt.component.html',
  styleUrls: ['./tyreregrouprcvdrpt.component.css']
})
export class TyreregrouprcvdrptComponent {
...
dashboard: string =""; 
  ...
  ngOnInit(): void {   
    ...
    var privilegeStatus = menuTypeList.flatMap((item: { menuList: any; }) => item.menuList)
      .find((aa: { menuName: string; }) => aa.menuName === "Tyre Receive from Regroup");
      if (privilegeStatus) {
  ...
  getTyreStatus(){
    this.dtOptions = {
        ...
        ajax: (dataTablesParameters: any, callback) => {
          ...
          this.tyremgntrptService.getTyreReGroupRcvdRptList(this.filter).subscribe(resp => {
  ...
  exportExcel(): void {      
    ...
    this.tyremgntrptService.getTyreReGroupRcvdRptExcel(this.filter).subscribe(resp => {
      if(resp.status){      
        let link = document.createElement("a");
        link.download = "TyreRegroupRcvd" + "_" + new Date().getTime() + '.xlsx';
