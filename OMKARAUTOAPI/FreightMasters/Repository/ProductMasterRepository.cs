...
namespace FreightMasters.Repository
{
    public class ProductMasterRepository : IProductMasterRepository
    {
        ...
        public async Task<ResponseModel> CheckDuplicateProduct(RequestModel requestModel)
        {
            ResponseModel responseModel = new();
            ...
            try
            {
                if (dbconnection != null)
                {
                    SqlParameter[] param =
                        {
                            new SqlParameter("@ProductName", requestModel.strRequest),
                          //  new SqlParameter("@ClassDesc", requestModel.strRequest1),
                        };
                    var statusData = await SqlHelper.SqlHelper.ExecuteDatasetAsync(transaction, "usp_ChkDuplicateProduct", param);
