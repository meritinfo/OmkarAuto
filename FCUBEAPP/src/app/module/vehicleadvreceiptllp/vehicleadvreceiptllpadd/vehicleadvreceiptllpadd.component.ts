...
@Component({
  selector: 'app-vehicleadvreceiptllpadd',
  templateUrl: './vehicleadvreceiptllpadd.component.html',
  styleUrls: ['./vehicleadvreceiptllpadd.component.css']
})
export class VehicleadvreceiptllpaddComponent {
...
dashboard: string ="";
  ...
  selectedvehicleAdvBalReceiptDetail = new VehicleadvbalreceiptModelLLP();

  constructor(private route: Router, private formBuilder: FormBuilder, 
    private vehicleadvbalreceiptModel: VehicleadvbalreceiptModelLLP, 
    private vehiclerepmaintMasterService:VehicleadvbalreceiptServiceLLP, 
    private sharedService : SharedService,
    private commonService: CommonService,private toastrService: ToastrService,
    private requestmodel:Requestmodel) {
    this.vehicleadvbalreceiptModel = new VehicleadvbalreceiptModelLLP();
  ...
  searchStatement(){
    ...
    if (selectedDataValue.partyId.dataId) {
      ...
      this.toastrService.warning("Please Select Valid Party");
      return;
    }  

    this.requestmodel.strRequest = selectedDataValue.tripsUptoDate ; 
    this.requestmodel.strRequest1 = selectedDataValue.partyId.dataId; 
    ...
    this.requestmodel.strRequest =this.selectedvehicleAdvBalReceiptDetail.transId 
      if (confirm("Are you sure, you want to delete this?")) {
            ...
            if (this.responseDetails.status) {
              ...
              this.formUser.reset();
              this.route.navigate(['/vehicleadvballistbrpl']);
            }
            else {
  ...
  submitVehicleRepMaintMasterForm(): void {
    ...
    this.vehicleadvbalreceiptModel.partyId = selectedDataValue.partyId.dataId?selectedDataValue.partyId.dataId:'';
    ...
    for (var i = 0; i < selectedDataValue.arrayList.length; i++) {
            ...
            + parseFloat(selectedDataValue.arrayList[i].extras)) >0 ) 
      {
        this.vehicleadvbalreceiptModel.vehicleAdvBalReceiptDtlList.push({
          'loadId': selectedDataValue.arrayList[i].loadId,
          'loadBranch': "",
          'loadMemoNo': "",
          'loadDate': "",
           'fromPlace': "",
