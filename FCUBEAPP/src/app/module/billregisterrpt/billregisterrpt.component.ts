...
import { BillregisterrptService } from 'src/app/services/billregisterrpt.service';
import { ExcelService } from 'src/app/services/excel.service';
import { Responsemodel } from 'src/app/models/responsemodel';
import { ToastrService } from 'ngx-toastr';

@Component({
  selector: 'app-billregisterrpt',
  templateUrl: './billregisterrpt.component.html',
  styleUrls: ['./billregisterrpt.component.css']
})
export class BillregisterrptComponent {
  ...
  allBillregisterrptlist: Billregisterrptlistmodel = new Billregisterrptlistmodel();
  filter: Reportmodel = {
    pageNumber: 1,
    pageSize: 10,
  ...
  rptType= true;
  constructor(private billregisterrptService: BillregisterrptService, 
    ...
    billregisterList(){
      this.dtOptions = {
        ...
        ajax: (dataTablesParameters: any, callback) => {
          ...
          this.billregisterrptService.getBillregisterrptList(this.filter).subscribe(resp => {
            this.allBillregisterrptlist = resp; 
              callback({
    ...
    //  this.filter.filterStr3  = selectedDataVal.destination?selectedDataVal.destination.dataId:"";
      this.billregisterrptService.getBillregisterrptExcel(this.filter).subscribe(resp => {
      
        if(resp.status){      
          let link = document.createElement("a");
          link.download = "Bill Register" + "_" + new Date().getTime() + '.xlsx';
