...
@Component({
  selector: 'app-trippaymentsllpadd',
  templateUrl: './trippaymentsllpadd.component.html',
  styleUrls: ['./trippaymentsllpadd.component.css']
})
export class TrippaymentsllpaddComponent {
  ...
  ngOnInit(): void {
    ...
    this.formTripPayment = this.formBuilder.group({
      ...
      withLRYN: new FormControl('Y', [Validators.required]),
     ...
     this.formTripPayment.controls['toPlace'].disable();
      ...
      //  this.changeTransTypeNew(this.selectedTripPaymentsDetails.transType);
        ...
        this.formTripPayment.patchValue({
          ...
          bookingDate:  this.commonService.formatDate(this.selectedTripPaymentsDetails.bookingDate), 
  ...
  getLRDetails(vehiNo: string){
    var selectedData = this.formTripPayment.getRawValue();
    this.requestmodel.strRequest = vehiNo;
    this.requestmodel.strRequest1 = selectedData.pmtDate;

    this.tripPaymentsService.getLrDtlsForTripPmts(this.requestmodel).subscribe((res: Consignmentmodel) => {
      if(res.consignmentID){
        this.consignmentId = res.consignmentID;
        this.formTripPayment.patchValue({
          gcNoteNo: res.gcNoteNo,    
          bookingDate: this.commonService.formatDate(res.bookingDate),    
          fromPlace: res.fPlace,    
          toPlace: res.tPlace,    
        });
      }
      else{
  ...
  calculateTotalAmount() {
    ...
    var selectedData = this.formTripPayment.getRawValue();
    var ratePerLtr = 0;
    if (selectedData.qtyLtrs!= "" && selectedData.amountPaid!= "") {
      var qtyLtrs = selectedData.qtyLtrs ? parseFloat(selectedData.qtyLtrs) : 0;
      var amountPaid = selectedData.amountPaid ? parseFloat(selectedData.amountPaid) : 0;
      ratePerLtr = (amountPaid/qtyLtrs);   
    }
    this.formTripPayment.patchValue({
      ratePerLtr: ratePerLtr.toFixed(2),    
    });
  }
  ...
  submitTripPaymentsForm(): void {  
    ...
    let d4 = new Date(selectedDataValue.pmtDate);
    if (d3>d4 || d2<d4 ) {
      this.formTripPayment.patchValue({
        pmtDate: ''
      });
    ...
    formData.append('attachment1', this.attachment1Input.nativeElement.files[0]);
    formData.append('attachment2', this.attachment2Input.nativeElement.files[0]);
