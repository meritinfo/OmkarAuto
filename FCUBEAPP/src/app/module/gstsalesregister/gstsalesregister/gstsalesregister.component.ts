...
@Component({
  selector: 'app-gstsalesregister',
  templateUrl: './gstsalesregister.component.html',
  styleUrls: ['./gstsalesregister.component.css']
})
export class GstsalesregisterComponent { loggedInUserID: string = '';
  createStatus = false;
  editStatus = false;
  deleteStatus = false;
  viewStatus = false; 
  ...
  ngOnInit(): void {     
    ...
    if (typeof menuData !== 'undefined' && menuData !== null && menuData !== '') {
      ...
      .find((aa: { menuName: string; }) => aa.menuName === "GST Register (Bill)");
      if (privilegeStatus) {
  ...
  exportExcel(): void {      
    ...
    this.bookingregisterService.getGstSalesRptExcel(this.filter).subscribe(resp => {      
      if(resp.status){      
