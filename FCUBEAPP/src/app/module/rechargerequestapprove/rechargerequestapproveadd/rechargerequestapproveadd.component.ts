...
import { FormArray, FormBuilder, FormControl, FormGroup, Validators } from '@angular/forms';import { Router } from '@angular/router';
...
rechargeRequestAppSave(): void {
  ...
  var rechargerequestarray = this.rechargerequestlist.rechargeRequestLst;
    ...
    for (var i = 0; i < rechargerequestarray.length; i++) {
      if (rechargerequestarray[i].selected) {
           if(arr[i].approvedYN=="")
           {
            this.toasterService.warning("Please select approval: Y or N");
            return;
           }
            if(arr[i].approvedYN=="Y")
            {
  ...
  this.rechargerequestService.rechargeRequestAppSave(this.rechargerequestlist)
