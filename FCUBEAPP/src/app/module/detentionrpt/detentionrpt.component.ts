...
@Component({
  selector: 'app-detentionrpt',
  templateUrl: './detentionrpt.component.html',
  styleUrls: ['./detentionrpt.component.css']
})
export class DetentionrptComponent {
  interval:any;
...
dashboard: string =""; 
  ...
  allDprrptlist: Detentionrptlistmodel = new Detentionrptlistmodel();
  filter: Reportmodel = {
    pageNumber: 1,
    pageSize: 10,
  ...
  constructor(private unbilledrptService: FreightreportsService, 
    ...
    ngOnInit(): void {     
      ...
      if (typeof menuData !== 'undefined' && menuData !== null && menuData !== '') {
        ...
        .find((aa: { menuName: string; }) => aa.menuName === "Detention Report");
        if (privilegeStatus) {
    ...
    dprrptlist(){
      this.dtOptions = {
        ...
        ajax: (dataTablesParameters: any, callback) => {
          ...
          this.unbilledrptService.getDetentionRptList(this.filter).subscribe(resp => {
    ...
    if (maxdt<frmdt || frmdt<mindt || maxdt<todt || todt<mindt) {
      ...
      this.filter.filterStr2 = selecteddata.origin?selecteddata.origin.dataId:"";
      this.filter.filterStr3 = selecteddata.destination?selecteddata.destination.dataId:"";

      this.unbilledrptService.getDetentionRptExcel(this.filter).subscribe(resp => {
      
        if(resp.status){      
          let link = document.createElement("a");
          link.download = "DetentionReport" + "_" + new Date().getTime() + '.xlsx';
