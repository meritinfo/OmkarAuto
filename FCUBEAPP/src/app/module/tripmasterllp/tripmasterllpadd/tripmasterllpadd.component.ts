...
@Component({
  selector: 'app-tripmasterllpadd',
  templateUrl: './tripmasterllpadd.component.html',
  styleUrls: ['./tripmasterllpadd.component.css']
})
export class TripmasterllpaddComponent {
  ...
  constructor(private route: Router, private formBuilder: FormBuilder, 
    private tripsheetmodel: Tripmastermodel, private tripSheetService: TripsheetllpService, 
  ...
  refreshFrieght(){
    ...
    this.tripSheetService.getTripMasterInnerGridRefreshList(this.requestmodel).subscribe((res: Tripmastermodel) => {     
      for (let i = 0; i < res.routeList.length; i++) {
        this.formRouteArray.push(this.createRouteArray());
        this.formRouteArray.controls[i].get("loadId")?.setValue(res.routeList[i].loadId);
        this.formRouteArray.controls[i].get("loadBranch")?.setValue(res.routeList[i].loadBranch);
  ...
  deleteTripsheetForm(): void {
    ...
    if (confirm("Are you sure, you want to delete this?")) {
      this.tripSheetService.tripMasterDetailsLlpDelete(this.requestmodel).subscribe((res: Responsemodel) => {
        this.responseDetails = res;
        if (this.responseDetails.status) {
          this.toastrService.success(this.responseDetails.message);
          this.formTripsheet.reset();
          this.route.navigate(['/tripsheetllplist']);
        }
        else {
          this.toastrService.warning(this.responseDetails.message);
        }
  ...
  onFastAmt(){    
    ...
    for (let i = 0; i < selectedDataValue.fasttagList.length; i++) {
      if(selectedDataValue.fasttagList[i].ftAmount!=''){
        fastagAmount = fastagAmount + parseFloat(selectedDataValue.fasttagList[i].ftAmount);
      }
    }
    this.formTripsheet.patchValue({
  ...
  submitTripSheetForm(): void {
    ...
    this.tripSheetService.tripMasterDetailsLlpSubmitted(this.tripsheetmodel).subscribe((res: Responsemodel) => {
      this.responseDetails = res;
      if (this.responseDetails.status) {
