...
namespace FinanceMasters.Repository
{
    public class OpeningBalanceMasterRepository : IOpeningBalanceMasterRepository
    {
        ...
        public async Task<ResponseModel> OpeningBalUpdate(RequestModel req)
        {
            ResponseModel responseModel = new();
            var connection = new SqlConnection(dbconnection.Value.DBConnection);
            connection.Open();
            ...
            try
            {
                ...
                var statusData = await SqlHelper.SqlHelper.ExecuteDatasetAsync(transaction, "usp_OpeningBalUpdate", param);
